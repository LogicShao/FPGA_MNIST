...
def write_rom(path, module_name, mem_filename, depth):
...
def main():
    ...
    for name, values in files:
        ...
        write_mem(mem_path, values.tolist())
        rom_path = os.path.join(args.out_dir, f"{name}_rom.v")
        write_rom(rom_path, f"rom_{name}", os.path.basename(mem_path), len(values))
        print(f"Wrote {mem_path} and {rom_path}")
